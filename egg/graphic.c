...
typedef struct egg
{
    ...
    int   fX;             /**< egg左上角的x坐标（盘子坐标系）    */
    int   fY;             /**< egg左上角的y坐标（盘子坐标系）    */
    int   fPX;            /**< 当前画笔位置的x坐标（egg坐标系）  */
    int   fPY;            /**< 当前画笔位置的y坐标（egg坐标系）  */
...
void MovePen(int x, int y)
{
    ...
    MoveToEx(pActiveEgg->hdc, pActiveEgg->fPX = x, pActiveEgg->fPY = y, NULL);
}

void OffsetPen(int dx, int dy)
{
...
void DrawString(char* pString)
{
    ...
    TextOut(pActiveEgg->hdc, pActiveEgg->fPX, pActiveEgg->fPY, pString, _tcslen(pString));
...
int GetStringWidth(char* pString)
{
    SIZE size;
    InitCheck();
    assert(pActiveEgg != NULL);
...
HEGG LayEgg(void)
{
    ...
    //初始画笔位置和初始egg位置都在左上角
    pEgg->fX = pEgg->fPX = rcPlate.left;
    pEgg->fY = pEgg->fPY = rcPlate.top;
...
void SetPen(unsigned long color, int fWidth)
{
    HPEN hPen;
    ...
    hPen = CreatePen(PS_SOLID, fWidth, color);
